<template>
  ...
  >
    <p class="text-sm text-on-surface-variant">
      Search active for <span class="font-bold text-on-surface">"{{ query }}"</span>. Showing
      <span class="font-bold text-primary">{{ shownCount }}</span> of
      <span class="font-bold text-on-surface">{{ totalCount }}</span> {{ entityLabel }}.
    </p>
    <button
      type="button"
