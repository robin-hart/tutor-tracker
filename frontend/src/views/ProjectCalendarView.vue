    ...
    <MainTopBar v-model="searchText" search-placeholder="Search slots...">
      <template #tabs>
        <nav class="flex gap-8">
          ...
          >
          <RouterLink
            :to="{ name: 'student-management', params: { projectId } }"
            class="text-on-surface-variant font-manrope text-sm"
            >Students</RouterLink
          >
        </nav>
      </template>
    ...
    <main class="ml-72 pt-32 px-12 pb-20">
      ...
      <div class="grid grid-cols-12 gap-8 mb-10">
        ...
        <div class="col-span-12 md:col-span-6 bg-surface-container-lowest p-8 rounded-xl shadow-sm">
          ...
          <div class="flex items-baseline gap-2">
            <span class="text-5xl font-black font-headline text-primary">{{
              formattedMonthHours.hours
            }}</span>
            <span class="text-on-surface-variant font-medium">hrs</span>
            <span class="text-3xl font-black font-headline text-primary">{{
              formattedMonthHours.minutes
            }}</span>
            <span class="text-on-surface-variant font-medium">min</span>
          </div>
        </div>
        ...
        <div class="bg-surface-container-lowest rounded-2xl p-8 shadow-sm">
          <div class="flex items-center justify-between mb-6 gap-4">
            ...
            <span class="text-sm text-on-surface-variant"
              >{{ filteredSlotResults.length }} slot{{
                filteredSlotResults.length === 1 ? '' : 's'
              }}
              found</span
            >
          </div>
          ...
          <div class="space-y-8">
            <section
              v-for="monthGroup in groupedFilteredSlots"
            ...
            >
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs text-on-surface-variant font-bold"
                  >{{ monthGroup.slots.length }} slot{{
                    monthGroup.slots.length === 1 ? '' : 's'
                  }}</span
                >
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                <article
                  v-for="slot in monthGroup.slots"
                  :key="slot.id"
                  class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-primary"
                >
                  <div class="flex justify-between items-start mb-2 gap-3">
                    <div>
                      <p class="text-[11px] text-on-surface-variant font-bold mb-1">
                        {{ slot.date }}
                      </p>
                      <h4 class="font-bold text-sm">{{ slot.title }}</h4>
                    </div>
                    <span
                      class="text-[10px] bg-primary/10 text-primary px-2.5 py-1 rounded-full font-black"
                      >{{ formatTime(slot.startTime) }}</span
                    >
                  </div>
                  <p class="text-[11px] text-on-surface-variant font-bold mb-2">
                    {{ formatPeriod(slot.startTime, slot.durationMinutes) }}
                  </p>
                  <p class="text-xs text-on-surface-variant">
                    {{ slot.description || 'No details provided.' }}
                  </p>
          ...
          <p
            v-if="!isLoading && filteredSlotResults.length === 0"
            class="text-sm text-on-surface-variant mt-6"
          >
      ...
      <div v-else class="grid grid-cols-12 gap-8">
        <div
          class="col-span-12 lg:col-span-8 bg-surface-container-lowest rounded-2xl p-8 shadow-sm"
        >
          <div class="flex items-center justify-between mb-8 gap-4">
            <h3 class="text-2xl font-black font-manrope">{{ monthLabel }}</h3>
            <div class="flex items-center gap-2">
              <button
                class="px-3 py-2 bg-surface-container-low rounded-lg text-sm font-bold"
          ...
          <div class="grid grid-cols-7 gap-px bg-outline-variant/20 rounded-xl">
            <div
              ...
              :key="day"
              class="bg-surface-container-low/30 text-center text-[10px] font-black uppercase tracking-widest py-3"
            >
              {{ day }}
            </div>
            <button
              ...
              :class="[
                selectedDate === cell.dateIso
                  ...
                  : cell.inMonth
                    ? 'bg-white hover:bg-surface-container-low/40'
            ...
            >
              ...
              <div
                ...
                "
              >
                {{ slotCountByDate[cell.dateIso] }} slot{{
                  slotCountByDate[cell.dateIso] > 1 ? 's' : ''
                }}
              </div>
            </button>
          </div>
          <p v-if="isLoading" class="text-sm text-on-surface-variant mt-4">
            Loading project slots...
          </p>
        ...
        <div class="col-span-12 lg:col-span-4 bg-surface-container-low rounded-2xl p-8">
          ...
          <p
            v-if="!isLoading && selectedDaySlots.length === 0"
            class="text-sm text-on-surface-variant"
          >
            No timeslots for this day yet.
          </p>
          <p class="text-xs text-on-surface-variant mt-4">
            Select a day in the calendar and use the plus button to add a timeslot to that exact
            date.
          </p>
        </div>
      </div>
      ...
      >
        ...
        <strong>{{ slotToDelete?.title || 'this timeslot' }}</strong>
        <span v-if="slotToDelete"
          >({{ formatPeriod(slotToDelete.startTime, slotToDelete.durationMinutes) }})</span
...
const monthLabel = computed(() =>
  new Intl.DateTimeFormat('en-US', { month: 'long', year: 'numeric' }).format(activeMonth.value)
);
