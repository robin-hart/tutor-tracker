...
public class TutorDataService {
    ...
    public List<StudentProfile> getProjectStudents(String projectId) {
        ...
        public List<ProjectGroupSummary> getProjectGroups(String projectId) {
                ...
                List<ProjectGroupEntity> groups = projectGroupRepository.findByProject(project);
                Map<String, Integer> counts = projectStudents.stream()
                ...
                return groups.stream()
                                .map(group -> new ProjectGroupSummary(group.getName(), counts.getOrDefault(group.getName(), 0)))
         ...
         */
        public ProjectGroupSummary createProjectGroup(String projectId, ProjectGroupCreateRequest request) {
                ProjectEntity project = findProjectBySlug(projectId);
        ...
        public void deleteProjectGroup(String projectId, String rawGroupName) {
                ...
                ProjectGroupEntity group = projectGroupRepository.findByProjectAndName(project, groupName)
                                .orElseThrow(() -> new IllegalArgumentException("Group not found: " + groupName));

                List<StudentEntity> assignedStudents = studentRepository.findByProjectAndGroupName(project, groupName);
        ...
        public List<TimeslotResponse> getProjectTimeslots(String projectId, String monthKey) {
                ...
                LocalDate from = month.atDay(1);
                LocalDate to = month.plusMonths(1).atDay(1);
                return timeslotRepository.findByProjectAndDateGreaterThanEqualAndDateLessThan(project, from, to).stream()
    ...
    public ReportRow generateProjectMonthlyReport(String projectId, String monthKey) {
        ...
        List<TimeslotEntity> slots = timeslotRepository.findByProjectAndDateGreaterThanEqualAndDateLessThan(project, from, to);
        ...
        ReportRowEntity report = reportRepository.findByProjectAndMonthKey(project, month.toString())
     ...
     */
         ...
         */
        public TimeslotResponse updateProjectTimeslot(String projectId, String timeslotId, TimeslotCreateRequest request) {
                ProjectEntity project = findProjectBySlug(projectId);
                TimeslotEntity slot = findTimeslot(project, timeslotId);
    ...
    private ProjectEntity findProjectBySlug(String projectId) {
        return projectRepository.findBySlug(projectId)
        ...
        private TimeslotEntity findTimeslot(ProjectEntity project, String timeslotId) {
                return timeslotRepository.findByProjectAndId(project, timeslotId)
        ...
        private void refreshProjectMetrics(ProjectEntity project) {
                ...
                double totalHours = timeslotRepository.findByProjectAndDateGreaterThanEqualAndDateLessThan(project, LocalDate.of(1970, 1, 1), LocalDate.of(2999, 1, 1))
                                .stream()
                                .mapToInt(TimeslotEntity::getDurationMinutes)
                                .sum() / 60.0;

                double monthHours = timeslotRepository.findByProjectAndDateGreaterThanEqualAndDateLessThan(project, from, to)
                                .stream()
                                .mapToInt(TimeslotEntity::getDurationMinutes)
                                .sum() / 60.0;
