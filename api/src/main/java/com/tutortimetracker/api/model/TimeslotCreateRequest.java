...
public record TimeslotCreateRequest(
        @Schema(description = "Session title", example = "Integration Workshop")
        @NotBlank String title,
        @Schema(description = "Optional details", example = "Focused revision session")
        String description,
        @Schema(description = "Session duration in minutes (minimum 15)", example = "90")
        @Min(15) int durationMinutes,
        @Schema(description = "Session date", example = "2026-03-20")
        @NotNull LocalDate date,
        @Schema(description = "Session start time", example = "14:00")
